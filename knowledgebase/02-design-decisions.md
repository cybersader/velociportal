...
- Tailscale SaaS loads OAuth plus `/tailnet/-/acl`, `/users`, and `/devices?fields=all`.
...
A username-triggered accessible settings panel replaces the former standalone logo toggle. Its one preference — hide or show the fixed built-in Velociportal logo — lives per exact identity in one browser only, scoped by an opaque SHA-256 digest of the login (never the raw login) and stored only in that browser's `localStorage`; a legacy unscoped key migrates once into the scoped key, and the preference is never sent to or stored by Velociportal. Optional strict `PORTAL_LOGO_DEFAULT=visible|hidden` supplies only the initial deployment default applied when no valid browser preference exists for that identity; the per-identity browser choice always wins once set. Arbitrary per-service logos, access-history collection, broader personalization, account-synchronized profiles, and delegated administration remain deferred. NPM `meta.nginx_online` is route/configuration state, not backend health. Future DNS/Tailscale hostname observations may become privacy-minimized, memory-only operator suggestions, but never authorization evidence or silent persisted mappings.
...
Machine cards are non-service-link summaries; D22 permits only the separately bounded browser-console navigation action. A card shows a short display/search name — the first DNS label of an already validated canonical `*.ts.net` target, re-derived only from that target and never from a raw node hostname or hostname-suggestion candidate — alongside the full canonical target; a validated IP fallback target has no separate short form. SSH policy renders as truthful, plain-language action text (`accept` → "No extra sign-in"; `check` → "Reauthenticate every `<period>`," using Tailscale's documented 12-hour default when `checkPeriod` is absent) rather than raw policy tokens. A copy command for a validated literal account is emitted server-side only; targets are restricted to canonical full `*.ts.net` names or validated Tailscale addresses so arbitrary device-reported FQDNs cannot redirect the copied command. `autogroup:nonroot` never invents a server-side command, but where it applies the card additionally offers a purely client-side text input: the viewer may type a candidate account, validated in the browser against the same bounded grammar as literal accounts (also rejecting `root`), before a command is built and copied — the typed value is never sent to or verified by Velociportal. HTML, JavaScript, and shell metacharacters remain escaped or rejected. Doctor exposes only state/reason/rule counts and per-identity machine counts, never device IDs, names, addresses, accounts, or commands. Live two-identity card/command/reachability parity is required while Tailscale remains preview.

## D21 — Production stack preflight is offline and non-mutating

`velociportal doctor --stack-env` validates the non-secret production Compose interpolation file without provider credentials, registry access, Docker inspection, TrueNAS access, or upstream calls. Same-name process-environment values are reported and applied with Docker Compose precedence so the check does not silently validate file values that the deployment shell would override. It rejects missing, untagged, `latest`, or malformed image references; accepts a syntactically valid SHA-256 digest pin; warns for other explicit tags; validates subnet/gateway containment; and requires a valid trusted-proxy CIDR while warning on broad or gateway-mismatched values.
...
## D23 — SSH account suggestions are browser-local, per-identity, and bounded

Where the D20 safe custom non-root account input applies, the portal remembers up to 10 validated account names the current browser has typed for the current identity, most-recently-used first, as a datalist suggestion source. Storage reuses the exact opaque SHA-256 login-digest scope already established for the browser-local logo preference (never the raw login) and lives only in that browser's `localStorage`; a visible control in the same settings panel clears the saved list for that identity/browser. Entries are validated against the same bounded account grammar before being remembered, and `root` is never remembered because the input only ever applies to the `autogroup:nonroot` scope.

Velociportal never receives, logs, or synchronizes these values — there is no server route, no cross-browser or cross-identity sharing, and no account-existence claim. This mirrors D-series appearance-preference handling: a convenience for the viewer's own browser, never state Velociportal or any other viewer can see.

## D24 — Mobile navigation and PWA support cannot cache portal state

The mobile shell adds a fixed safe-area-aware Services/Machines/More bottom navigation outside the htmx swap boundary. Services and Machines retain real hash-link fallbacks; Machines is hidden only when the complete projection is unavailable, while an available zero-match projection keeps its navigation destination. More opens the existing account settings dialog as a mobile bottom sheet rather than creating a second settings surface.

All identity-derived portal responses carry `Cache-Control: no-store, max-age=0`, `Pragma: no-cache`, and `Vary` for the trusted Tailscale identity headers. The embedded manifest and committed PNG icons provide install metadata, but the service worker intentionally implements only install/activate lifecycle handling. It has no fetch handler, Cache Storage writes, offline fallback, or cached authorization-derived HTML/cards. Full service-worker control and ordinary install prompts require HTTPS or localhost; the canonical plain-HTTP Tailscale Serve route remains valid over WireGuard but does not provide a secure context. Enabling HTTPS Serve/private TLS is a separate infrastructure decision, not part of the portal code.

## D25 — Browser-only search stays outside the swap boundary; health labels describe backend checks, not access

The mobile bottom dock (D24) replaces font-glyph icons with four embedded, decorative SVG line icons (`aria-hidden`, `focusable="false"`) inside an inset rounded surface. Services/Machines remain real section links carrying `aria-current="location"`, tracked by a feature-detected `IntersectionObserver` with a click fallback that rebinds after every htmx swap and clears state when Machines disappears; Search and More stay buttons and never carry `aria-current` or an `href`, since they are actions, not locations. The dock still collapses to three items when the Machines projection is unavailable, and it hides — without ever hiding the focused control itself — while any mobile text input, including the D20/D23 custom SSH account field, holds focus, restoring on blur instead of measuring keyboard height.

Search is one visibly labeled `type="search"` field placed immediately before `#portal-content`, outside the `hx-select="#portal-content"` 60-second swap region, so the query and focus survive every refresh untouched and never need to be reapplied from storage. It filters only already-rendered, already-authorized DOM: service display names and their existing link hostname, plus machine short/full names and targets. It never indexes policy details, account history, or unrendered topology, and it never writes the query to storage, the URL, logs, network calls, or the service worker. An explicit `.card[hidden]`-style CSS rule wins over card/grid `display` rules; empty category groups hide during filtering; a distinct "No results for this search" element stays separate from the real policy-empty/zero-authorization states; and a `<noscript>` block hides the search affordances so a no-JS session never sees a nonfunctional control. On `htmx:afterSwap` the same query reapplies to the newly authorized DOM.

Service-status badges (D18) keep every enum value, probe target, classification rule, coarse result field, and stale-transition behavior unchanged; only the human-facing label changes, from access-sounding words ("reachable", "authentication required") to backend-check-scoped ones: Reachable → "Backend check passed", AuthRequired → "Backend denied", ResponseError → "Unexpected response", Unreachable → "Check failed", Unknown → "Check unknown", Stale → "Check stale". A passed check uses the existing neutral accent color rather than a green success color, because a TCP connect-and-close or an accepted HTTP status code is not proof a human browser session will see a usable page. A new native `<details>` disclosure next to the Services heading — rendered only when a health store is configured — spells this out in prose (TCP only opens a connection; an accepted status is not a usable-page proof; a 401/403 denial is not assurance that signing in will resolve it) without inventing status codes in the per-badge label itself, and without claiming to explain or fix any specific service's observed behavior.
