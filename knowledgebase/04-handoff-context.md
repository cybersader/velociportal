...
**Published `v0.2.0-rc.7` is immutable at `ghcr.io/cybersader/velociportal@sha256:455de7634379863ac24cc541b9afb2f2f323f2edbb19073ae65ca358401bc925`.** Live TrueNAS environment files reference RC.7 and portal health is good. The portal showed the RC.7 organization/logo markers, 47 service cards, an available Machines section, 10 machine cards, and 11 SSH copy controls; the exact running-container digest has not been independently captured.
...
The current feature branch adds a role-gated browser SSH console action on eligible Tailscale machine cards: a direct member holding an automatic-admin-equivalent role (Owner, Admin, IT admin, or Network admin) sees a link that opens the fixed filtered `https://console.tailscale.com/admin/machines?q=<validated target>` page in a new tab, using a target that passes the same narrow validation as the copy commands. It is role-gated navigation only, never a direct session, proxy, enforcement check, or reachability claim — Tailscale remains responsible for console eligibility, reauthentication, SSH policy, device posture, account choice, and the session itself. It is hidden for ineligible roles, Headscale, and an unavailable Machines projection, and it adds no new endpoint, scope, credential, or tailnet setting. `MachineCard` and all SSH/Grant evidence are unchanged.
