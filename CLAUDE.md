...
**Published `v0.2.0-rc.4` is immutable at `ghcr.io/cybersader/velociportal@sha256:30a7567c169836e8ae6bbf6c2280227d403b52c79a760ee57a2764d154fae02d`.** Live TrueNAS use confirmed the corrected Owner-to-Admin automatic membership, complete snapshot, Serve route, portal health, and 48 cards without changing the live policy. It also exposed two presentation defects: wildcard-only NPM names became `%2A` links, and NPM route state looked like backend health. Branch `fix/truthful-service-cards` implements first-concrete-domain selection, visible unlinked wildcard cards, no NPM-derived health dots, and strict optional presentation metadata. Every published RC is immutable and must never be replaced or retagged.
