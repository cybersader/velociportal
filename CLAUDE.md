...
- **Machine Grant sources do not become humans.** Valid source tags, IPs, CIDRs, host aliases, and supported machine/role autogroups may load so machine rules do not block the snapshot, but they never match a browser identity. `tagOwners` and tags on a user's nodes do not make the user a `tag:*` source. Destination tags still resolve through node addresses.
