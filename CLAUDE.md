...
**Task 31 architecture is implemented in the working tree; real acceptance remains pending.** The production bundle creates the named internal Docker network `velociportal-upstreams`; Velociportal uses direct HTTP to the exact Headscale and NPM aliases on that network, while all other Headscale locations require verified HTTPS. Browser ingress uses the existing host-network TrueNAS Tailscale app with declarative HTTP Serve on tailnet port `8081`, forwarding to the loopback-only Velociportal host port. Existing NPM provides the trusted HTTPS Headscale control/API endpoint before clients join the tailnet and proxies to Headscale over the internal network. The separate sibling `../headscale-ops` remains workstation-only and HTTPS-only.
...
flowchart TD
    ...
    HS -->|"internal HTTP runtime API"| Cache["Complete in-memory snapshot"]
    NPM["NPM proxy-host API"] -->|"internal HTTP"| Cache
