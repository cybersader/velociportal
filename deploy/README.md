...
Tailscale exposes no standalone browser-SSH session URL. Every rendered machine requires supported SSH policy, Grant TCP/22, and exact device `sshEnabled=true` plus `blocksIncomingConnections=false`; missing or invalid capability removes the row and command. The fixed Machines-page action additionally requires an automatic-admin-equivalent direct-member role (Owner, Admin, IT admin, or Network admin). Its query uses a validated short name/IP plus `property:tailscale-ssh`; the action remains navigation rather than a session, proxy, enforcement check, health signal, reachability claim, or success guarantee.
