...
- **Grant roles have no hierarchy or inference.** Supported human-role autogroups become browser evidence only for Grants and only through the authoritative Users API mapping. A direct `type: "member"` user receives `autogroup:member` plus exactly its API role; a `type: "shared"` user receives none. Membership requires exact `loginName` equality and is never inferred from devices, owners, tags, or `tagOwners`. Legacy ACLs and `nodeAttrs` do not consume this mapping.
...
- **Tailscale remains preview.** Immutable `v0.2.0-rc.2` contains safe Grants, but live acceptance exposed missing authoritative role membership and omitted expected role-derived cards. `v0.2.0-rc.3` is the next preview candidate; token refresh/revocation, role and owner mapping, two-identity parity, unsupported-policy negatives, and reachability still require live proof.
