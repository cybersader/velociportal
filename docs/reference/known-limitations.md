...
- **Grant roles use Tailscale's explicit automatic memberships only.** Supported human-role autogroups become browser evidence only for Grants and only through the authoritative Users API mapping. A direct `type: "member"` user receives `autogroup:member` plus its API role; the Owner additionally receives `autogroup:admin`. A `type: "shared"` user receives none, and specialized roles do not imply one another. Membership requires exact `loginName` equality and is never inferred from devices, owners, tags, or `tagOwners`. Legacy ACLs and `nodeAttrs` do not consume this mapping.
...
- **Tailscale remains preview.** Immutable `v0.2.0-rc.5` includes the Owner's automatic `autogroup:admin` membership, truthful wildcard cards, and bounded service health; live TrueNAS use produced 48 Owner cards. Token refresh/revocation, separate owner mapping, two-identity parity, unsupported-policy negatives, and actual reachability still require live proof.
...
- **Users and device mapping are intentionally strict.** The complete Users API response must contain canonical exact IDs, `loginName`, `type`, and `role` values before Grant-role membership is derived. Device-owner resolution remains a separate mapping; blank, duplicate, ambiguous, malformed, or unresolved data rejects the refresh. Tagged devices may omit a human owner; untagged devices may not. Neither mapping infers the other.
...
- **Full-domain identities match exactly.** A trusted `alice@example.com` login does not inherit ambiguous `alice@` or bare `alice` membership. Grant-role lookup also requires the exact Users API `loginName`; short, bare, cross-domain, and case-variant identities do not inherit role membership.
