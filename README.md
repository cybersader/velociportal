...
> [!NOTE]
> Published `v0.2.0-rc.12` is immutable at `sha256:370a2d2b70ee58e12c403f71a5f78056476ed45913814563a115fa302d1fa288`. Live TrueNAS environment files reference RC.12, and live rendering confirms the compact list is active; the exact running-container digest has not been independently captured. That rendering exposed inconsistent action columns, non-SSH devices receiving policy-only copy controls, and service-health labels squeezing service domains. The current branch requires explicit device SSH capability for every machine, aligns row actions, and gives service names a full wrapping line.
