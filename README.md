...
> [!NOTE]
> Published `v0.2.0-rc.11` is immutable at `sha256:52eb35182a0f1a7ec60c55d5f27dfb22561e9e0b88b953747303821854f652de`. Live TrueNAS environment files reference RC.11, and live portal rendering exposed that its narrow multi-column machine cards still bury primary SSH actions under repetitive policy text; the exact running-container digest has not been independently captured. The current branch replaces that grid with a compact responsive machine list while preserving all RC.11 authorization and privacy boundaries.
