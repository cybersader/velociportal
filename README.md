...
> [!NOTE]
> Published `v0.2.0-rc.8` is immutable at `sha256:d3423da915931f4f61b65ecbdfc94094407c0d34b16b2822b1798cec9563d9e5`. Live TrueNAS environment files reference RC.8, and live testing reached its browser SSH action and account settings; the exact running-container digest has not been independently captured. That testing exposed full-canonical-name console searches that returned zero results plus unclear machine-card account/action wording. The current correction branch implements the short-name search/display, plain-language reauthentication labels, and browser-local custom-account suggestions described below; it is not yet published or deployed.
...
Only a direct member holding an automatic-admin-equivalent role (Owner, Admin, IT admin, or Network admin) also sees a browser SSH console action on each eligible machine card. It opens the fixed filtered `https://console.tailscale.com/admin/machines?q=<value>` page in a new tab (`target=_blank rel="noopener noreferrer"`) using a target that passes the same narrow validation as the copy commands; because Tailscale's own Machines search matches the short machine name, a canonical target searches on its short name rather than the full canonical name, while a validated IP fallback target searches on the address itself. It is role-gated navigation, never a direct session, proxy, enforcement check, or reachability claim; Tailscale remains responsible for console eligibility, reauthentication, SSH policy, device posture, account choice, and the session itself. Billing admins, auditors, plain members, shared users, and Headscale never see this link.
