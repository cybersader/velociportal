...
> [!NOTE]
> `v0.2.0-rc.13` is tagged on `main`, merging the fixes rc.12's live rendering surfaced: explicit device SSH capability is now required for every machine card, row actions are aligned, and service names get a full wrapping line instead of being squeezed by health badges. RC.13 has not yet been published as an immutable image or exercised against the live TrueNAS environment; RC.12 remains the last independently confirmed running deployment. A separate branch adds mobile shell redesign, persistent browser-only search, and backend-check-scoped service-status wording on top of RC.13; it is implementation-complete but not yet published or deployed.
...
The Tailscale-preview Machines section is a separate visibility projection. It is available only when the selected provider is Tailscale and the complete `ssh` policy is present and supported; Headscale, an absent SSH section, or any unsupported SSH rule omits the entire Machines section. When the projection is available but the current identity has no matches, the portal preserves an explicit empty Machines state. A device appears only when the same exact direct member matches the bounded supported `ssh` policy subset, an independent Grant permits TCP/22 to that device, and the exact device currently reports `sshEnabled=true` plus `blocksIncomingConnections=false`. Missing or invalid capability evidence fails closed, so phones and other non-SSH devices are not offered commands merely because broad policy selectors match. Legacy ACL port 22, NPM hosts, service metadata, and health never create a machine row. Each compact row shows the short display/search name and canonical target, one `SSH as` workflow, a `Copy Tailscale SSH` action, and an optional Tailscale Machines action; exact account scopes and plain-language identity-check behavior remain available under collapsed `Policy details`. Copyable commands for validated literal accounts are built server-side and use a canonical full `*.ts.net` device name or validated Tailscale IPv4/IPv6 address. Where `autogroup:nonroot` applies, the same workflow can reveal a purely client-side, client-validated custom account input that is never pre-filled, sent to, or verified by Velociportal. Projection unavailability never broadens or invalidates otherwise supported HTTP service cards. The selected control plane, Tailscale Serve, Tailscale SSH, NPM, and backends still enforce access and reachability.

Tailscale does not provide a standalone one-click browser-SSH session URL. Velociportal can only open the admin console's filtered Machines page. Every rendered machine already requires the exact device to report `sshEnabled=true` and `blocksIncomingConnections=false`; the navigation action additionally requires the viewer to be a direct member holding an automatic-admin-equivalent role (Owner, Admin, IT admin, or Network admin). The fixed link uses `q=<validated-short-name-or-IP> property:tailscale-ssh`, opens in a new tab, and remains navigation rather than a session, proxy, enforcement check, health signal, reachability claim, or guarantee that browser SSH will succeed.
...
- Separate Tailscale-preview SSH Machines cards requiring a fully supported SSH rule, Grant TCP/22 evidence, and explicit current `sshEnabled=true` plus `blocksIncomingConnections=false` device capability for the same exact direct member and device; non-service-link summaries and narrowly validated copy commands only
